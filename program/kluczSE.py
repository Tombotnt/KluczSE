...
parser = argparse.ArgumentParser(
                    prog='kluczSE.py',
                    description='Program wykrywający klucze płasko-oczkowe')

parser.add_argument('source',help='źródło obrazu (0 dla kamerki)')
...
if args.source == '0':                      # program dla kamerki
    ...
    while capture.isOpened():
        ...
        if ret == True:                     # jeśli jest dostępna klatka można przetworzyć
            
            if args.gpu:                                                    # wykorzystanie GPU do obliczeń
                results = model(image, device='0')
            else:                                                           # wykorzystanie CPU do obliczeń
                results = model(image)
            bboxes = np.array(results[0].boxes.xyxy.cpu(), dtype="int")     # tablica z pozycjami prostokątów
            confs = np.array(results[0].boxes.conf.cpu(), dtype="float")     # tablica z klasami wykrytych obiektów

            for conf, bbox in zip(confs, bboxes):
                (x, y, x2, y2) = bbox
                cv2.rectangle(image, (x, y), (x2, y2), (0, 0, 225), 2)      # rysowanie prostokąta na obrazie
                cv2.putText(image, str(round(conf,2)), (x, y - 5), cv2.FONT_HERSHEY_PLAIN, 2, (0, 0, 225), 2)
            cv2.imshow('Obraz z kamery', image)                             # wyświetlenie obrazu
            if cv2.waitKey(25) & 0xFF == ord('x'):                          # zatrzymanie programu po wciśnięciu x
                break
        else:
            break
    ...
    if image.shape[0] > 640 or image.shape[1] > 800:    # skalowanie obrazu jeśli jest za duży
    ...
    bboxes = np.array(results[0].boxes.xyxy.cpu(), dtype="int")             # tablica z pozycjami prostokątów
    confs = np.array(results[0].conf.cls.cpu(), dtype="float")             # tablica z klasami wykrytych obiektów
    ...
    cv2.imshow("wynik", image)                                              # wyświetlenie obrazu
